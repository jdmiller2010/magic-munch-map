...
- **Theming is CSS custom properties** on `:root` with a `prefers-color-scheme: dark` override. New colors belong there, not inline — except marker/popup styling, which Leaflet injects outside the themed tree and therefore hardcodes hex values (`COLORS`, `.pop`).
